...
export class Get {
  public async userFollowing(req: Request, res: Response): Promise<void> {
    ...
    const cachedFollowees: IFollowerData[] = await followerCache.getFollowersFromCache(`followers:${req.currentUser!.userId}`);
  ...
  public async userFollowers(req: Request, res: Response): Promise<void> {
    ...
    const cachedFollowers: IFollowerData[] = await followerCache.getFollowersFromCache(`followees:${req.params.userId}`);
